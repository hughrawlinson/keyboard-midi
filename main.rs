...
fn main() {
    ...
    match run(score, speed) {
        Ok(_) => (),
        Err(err) => println!("Error: {}", err.description())
    }
}

fn run(score: [(u8, u8); 9], speed: u16) -> Result<(), Box<Error>> {
    // let duration = speed *
    let scoreDuration: u16 = speed * score.to_vec().iter()
        .fold(0, |sum, pair| {sum + pair.1}) as u16;
    let timescore = score.to_vec().iter()
        .fold(vec![(0,0)], |acc, el| {
            let mut v = acc.clone();
            match acc.last() {
                None => v.push((el.0 as u8, (el.1) as u16)),
                Some(w) => {
                    v.push((el.0 as u8, (el.1 as u16 + w.1 as u16) as u16))
                }
            };
            v
        });
    let abstimescore: Vec<(u8, u16)> = timescore.iter().map(|el| {
        (el.0, el.1 * speed)
    }).collect();
    let mut position = 0;
    ...
    println!("Connection open. Listen!");
    {
        ...
        while position < scoreDuration {
            ...
            if note.1 as i16 > lastnotetime {
                ...
                println!("note: {}, position: {}", note.0, (position as f32/scoreDuration as f32));
                play_note(note.0, (position as f32/scoreDuration as f32));
                // thread::spawn(move || {
